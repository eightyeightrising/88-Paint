        ...
        newHandle = await showSaveFilePicker({
          types: formats.map((format) => {
            return {
              ...
              accept: {
                [format.mimeType]: format.extensions.map(
                  (extension) => "." + extension
                ),
              },
            };
...
const add_image_format = (
  mime_type,
  name_and_exts,
  target_array = image_formats
) => {
  ...
  if (get_direction() === "rtl") {
    const rlm = "\u200F";
    const lrm = "\u200E";
    format.nameWithExtensions = format.nameWithExtensions.replace(
      ext_regexp,
      `${rlm}*.${lrm}$1${rlm}`
    );
  }
  ...
  return formats.filter(
    (format, format_index) =>
      !format.extensions.some((extension) =>
        formats.some(
          (other_format, other_format_index) =>
...
for (const [format_id, format] of Object.entries(AnyPalette.formats)) {
  if (format.write) {
    const inside_parens = format.fileExtensions
...
const $status_text = $(E("div"))
  .addClass("status-text status-field inset-shallow")
  .appendTo($status_area);
const $status_position = $(E("div"))
  .addClass("status-coordinates status-field inset-shallow")
  .appendTo($status_area);
...
const news_period = local_storage_unavailable
  ? news_period_if_cannot_dismiss
  : news_period_if_can_dismiss;
if (
  Date.now() <
    Date.parse(latest_news_datetime) + news_period * 24 * 60 * 60 * 1000 &&
  news_seen !== latest_news_datetime
) {
...
const color_ramp = (num_colors, start_hsla, end_hsla) =>
  Array(num_colors)
    .fill()
    .map((_undefined, ramp_index, array) => {
...
function update_fill_and_stroke_colors_and_lineWidth(selected_tool) {
  ...
  main_ctx.fillStyle =
    fill_color =
    main_ctx.strokeStyle =
    stroke_color =
      selected_colors[
        ctrl && selected_colors.ternary && pointer_active
          ? "ternary"
...
const eye_gaze_mode_config = {
  ...
  dwellClickEvenIfPaused: (target) => target.matches(".toggle-dwell-clicking"),
  shouldDrag: (target) =>
...
async function init_eye_gaze_mode() {
  if (enable_tracky_mouse) {
    ...
    TrackyMouse.onPointerMove = (x, y) => {
      ...
      if (target !== last_el_over) {
        if (last_el_over) {
          window.untrusted_gesture = true;
          const event = new /*PointerEvent*/ $.Event(
            "pointerleave",
            Object.assign(get_event_options({ x, y }), {
              button: 0,
              buttons: 1,
              bubbles: false,
              cancelable: false,
  ...
  const get_hover_candidate = (clientX, clientY) => {
    ...
    for (const {
      from,
      to,
      withinMargin = Infinity,
    } of eye_gaze_mode_config.retarget) {
      if (
        ...
        if (to_element === null) {
          ...
          if (
            ...
            hover_candidate.x = Math.min(
              to_rect.right - 1,
              Math.max(to_rect.left, hover_candidate.x)
            );
            hover_candidate.y = Math.min(
              to_rect.bottom - 1,
              Math.max(to_rect.top, hover_candidate.y)
            );
            retargeted = true;
          }
        }
  ...
  const update = () => {
    ...
    recent_points = recent_points.filter(
      (point_record) => time < point_record.time + averaging_window_timespan
    );
    if (recent_points.length) {
      ...
      const recent_movement_amount = Math.hypot(
        latest_point.x - average_point.x,
        latest_point.y - average_point.y
      );
      ...
      if (hover_candidate && !gaze_dragging) {
        const apparent_hover_candidate = get_hover_candidate(
          hover_candidate.x,
          hover_candidate.y
        );
        const show_occluder_indicator = (occluder) => {
          ...
          occluder_indicator.style.left = `${
            occluder_rect.left + outline_width
          }px`;
          occluder_indicator.style.top = `${
            occluder_rect.top + outline_width
          }px`;
          occluder_indicator.style.width = `${
            occluder_rect.width - outline_width * 2
          }px`;
          occluder_indicator.style.height = `${
            occluder_rect.height - outline_width * 2
          }px`;
        ...
        if (apparent_hover_candidate) {
          if (
            ...
            !eye_gaze_mode_config.isEquivalentTarget(
              apparent_hover_candidate.target,
      ...
      if (hover_candidate) {
        ...
        if (time > hover_candidate.time + hover_timespan) {
          if (pointer_active || gaze_dragging) {
            window.untrusted_gesture = true;
            hover_candidate.target.dispatchEvent(
              new PointerEvent(
                "pointerup",
      ...
      dwell_indicator.style.transform = `scale(${
        circle_radius / circle_radius_max
      })`;
      dwell_indicator.style.left = `${
        circle_position.x - circle_radius_max / 2
      }px`;
      dwell_indicator.style.top = `${
        circle_position.y - circle_radius_max / 2
      }px`;
      ...
      let halo_target =
        ...
        (
          hover_candidate ||
          get_hover_candidate(latest_point.x, latest_point.y) ||
          {}
        ).target;

      if (
        halo_target &&
        (!paused || eye_gaze_mode_config.dwellClickEvenIfPaused(halo_target))
      ) {
        ...
        while (ancestor instanceof HTMLElement) {
          ...
          if (ancestor_computed_style.transform) {
            ...
            const match = ancestor_computed_style.transform.match(
              /(?:scale|matrix)\((\d+(?:\.\d+)?)/
            );
            if (match) {
        ...
        halo.style.borderTopRightRadius = `${
          parseFloat(computed_style.borderTopRightRadius) * border_radius_scale
        }px`;
        halo.style.borderTopLeftRadius = `${
          parseFloat(computed_style.borderTopLeftRadius) * border_radius_scale
        }px`;
        halo.style.borderBottomRightRadius = `${
          parseFloat(computed_style.borderBottomRightRadius) *
          border_radius_scale
        }px`;
        halo.style.borderBottomLeftRadius = `${
          parseFloat(computed_style.borderBottomLeftRadius) *
          border_radius_scale
        }px`;
      } else {
        halo.style.display = "none";
      ...
      if (recent_movement_amount < 5) {
        if (!hover_candidate) {
          ...
          if (
            hover_candidate &&
            paused &&
            !eye_gaze_mode_config.dwellClickEvenIfPaused(hover_candidate.target)
          ) {
            hover_candidate = null;
          }
        }
  ...
  const $pause_button = $(`<button class="toggle-dwell-clicking"/>`)
    ...
    .on("click", () => {
      ...
      $pause_button.attr(
        "title",
        paused ? resume_button_text : pause_button_text
      );
    })
    .appendTo($floating_buttons)
    .append($("<div class='button-icon'>"));
...
$canvas.on("pointerdown", (e) => {
  ...
  if (pointers.length >= 1) {
    ...
    pointers = pointers.filter((pointer) => pointer.pointerId !== 1234567890);
    return;
  }
