((exports) => {
  ...
  const menus = {
    ...
    [localize("&Edit")]: [
      ...
      },
      {
        ...
        speech_recognition: ["show history", "history"],
        action: () => {
          show_document_history();
        },
        description: localize(
          "Shows the document history and lets you navigate to states not accessible with Undo or Repeat."
        ),
      },
      MENU_DIVIDER,
      {
    ...
    [localize("&Image")]: [
      // @TODO: speech recognition: terms that apply to selection
      {
        ...
        shortcut:
          window.is_electron_app && !window.electron_is_dev
            ? "Ctrl+R"
            : "Ctrl+Alt+R", // Ctrl+R reloads the browser tab (or Electron window in dev mode via electron-debug)
        speech_recognition: [
          "flip",
          "rotate",
          "flip/rotate",
      ...
      },
      {
        ...
        shortcut:
          window.is_electron_app || !looksLikeChrome ? "Ctrl+Shift+N" : "", // Ctrl+Shift+N opens incognito window in chrome
        speech_recognition: [
          "clear image",
          "clear canvas",
          "clear picture",
      ...
      },
      {
        ...
        action: () => {
          ...
          systemHooks.showSaveFileDialog({
            ...
            formats: palette_formats,
            getBlob: (format_id) => {
              const file_content = AnyPalette.writePalette(
                ap,
                AnyPalette.formats[format_id]
              );
  ...
  for (const [top_level_menu_key, menu] of Object.entries(menus)) {
    ...
    const add_literal_navigation_speech_recognition = (
      menu,
      ancestor_names
    ) => {
