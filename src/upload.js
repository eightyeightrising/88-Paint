((exports) => {
  ...
  function post_blob_message(blob) {
    window.parent.postMessage({ type: "paint", blob }, "*"); // TODO: SET TARGET ORIGIN
  }
  ...
  function show_uploader(blob) {
    ...
    const $preview_image_area = $(E("div"))
      .appendTo($imgur_window.$main)
      .addClass("inset-deep");
    ...
    const $upload_button = $imgur_window
      .$Button(
        "Upload",
        () => {
          URL.revokeObjectURL(blob_url);
          ...
          const parseImgurResponseJSON = (responseJSON) => {
            try {
              ...
              $imgur_status.text(
                "Received an invalid JSON response from Imgur: "
              );
            ...
            req.upload.addEventListener(
              ...
              (event) => {
                if (event.lengthComputable) {
                  ...
                  const percentage_text = `${Math.floor(
                    progress_value * 100
                  )}%`;
