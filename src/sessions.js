(() => {
  ...
  const canvas_has_any_apparent_image_data = () =>
    main_canvas.ctx
      .getImageData(0, 0, main_canvas.width, main_canvas.height)
  ...
  user.color_desaturated = `hsla(${user.hue}, ${~~(user.saturation * 0.4)}%, ${
    user.lightness
  }%, 0.8)`;
  ...
  class MultiUserSession {
    ...
    start() {
        ...
        (snap) => {
          ...
          if (uri == null) {
            ...
            img.onload = () => {
              ...
              const image_data_local = main_ctx.getImageData(
                0,
                0,
                main_canvas.width,
                main_canvas.height
      ...
      this._fb_listeners.forEach(
        ({ fb, event_type, callback /*, error_callback*/ }) => {
