...
function update_helper_layer_immediately() {
  ...
  if (!helper_layer) {
    helper_layer = new OnCanvasHelperLayer(
      0,
      0,
      main_canvas.width,
      main_canvas.height,
  ...
  const margin = 15;
  const viewport_x = Math.floor(
    Math.max($canvas_area.scrollLeft() / magnification - margin, 0)
  );
  ...
  const viewport_y = Math.floor(
    Math.max($canvas_area.scrollTop() / magnification - margin, 0)
  );
  const viewport_x2 = Math.floor(
    Math.min(
      viewport_x + $canvas_area.width() / magnification + margin * 2,
      main_canvas.width
    )
  );
  const viewport_y2 = Math.floor(
    Math.min(
      viewport_y + $canvas_area.height() / magnification + margin * 2,
      main_canvas.height
    )
  );
  ...
  if (thumbnail_canvas && $thumbnail_window.is(":visible")) {
    ...
    const scroll_width =
      main_canvas.clientWidth + padding_left - $canvas_area[0].clientWidth;
    const scroll_height =
      main_canvas.clientHeight + padding_top - $canvas_area[0].clientHeight;
    ...
    let scroll_x_fraction =
      $canvas_area[0].scrollLeft / Math.max(1, scroll_width);
    let scroll_y_fraction =
      $canvas_area[0].scrollTop / Math.max(1, scroll_height);
    ...
    let viewport_x = Math.floor(
      Math.max(
        scroll_x_fraction * (main_canvas.width - thumbnail_canvas.width),
        0
      )
    );
    let viewport_y = Math.floor(
      Math.max(
        scroll_y_fraction * (main_canvas.height - thumbnail_canvas.height),
        0
      )
    );
...
function render_canvas_view(
  ...
  const grid_visible =
    show_grid &&
    magnification >= 4 &&
    window.devicePixelRatio * magnification >= 4 &&
    is_helper_layer;
...
function update_disable_aa() {
  ...
  $canvas_area.toggleClass(
    "disable-aa-for-things-at-main-canvas-scale",
    dots_per_canvas_px >= 3 || round
  );
}
...
function set_magnification(new_scale, anchor_point) {
  ...
  // scale pencil and stroke sizes inversely with magnification < 1 or loaded images scaled down to fit within app window

  if (new_scale < 1) {
    window.pencil_size = (1 / new_scale) * default_pencil_size;
    window.stroke_size = (1 / new_scale) * default_stroke_size;
  } else {
...
function show_custom_zoom_window() {
  ...
  $w.$main.append(
    `<div class='current-zoom'>${localize("Current zoom:")} <bdi>${
      magnification * 100
    }%</bdi></div>`
  );
  ...
  $fieldset.append(`
		...
		<div class="fieldset-body">
			<input type="radio" name="custom-zoom-radio" id="zoom-option-1" aria-keyshortcuts="Alt+1 1" value="1"/><label for="zoom-option-1">${display_hotkey(
        "&100%"
      )}</label>
			<input type="radio" name="custom-zoom-radio" id="zoom-option-2" aria-keyshortcuts="Alt+2 2" value="2"/><label for="zoom-option-2">${display_hotkey(
        "&200%"
      )}</label>
			<input type="radio" name="custom-zoom-radio" id="zoom-option-4" aria-keyshortcuts="Alt+4 4" value="4"/><label for="zoom-option-4">${display_hotkey(
        "&400%"
      )}</label>
			<input type="radio" name="custom-zoom-radio" id="zoom-option-6" aria-keyshortcuts="Alt+6 6" value="6"/><label for="zoom-option-6">${display_hotkey(
        "&600%"
      )}</label>
			<input type="radio" name="custom-zoom-radio" id="zoom-option-8" aria-keyshortcuts="Alt+8 8" value="8"/><label for="zoom-option-8">${display_hotkey(
        "&800%"
      )}</label>
  ...
  const $really_custom_radio_option = $fieldset.find(
    "input[value='really-custom']"
  );
  const $really_custom_input = $fieldset.find(
    "input[name='really-custom-zoom-input']"
  );
  ...
  $w.$Button(
    localize("OK"),
    () => {
      let option_val = $fieldset
        .find("input[name='custom-zoom-radio']:checked")
        .val();
      let mag;
      if (option_val === "really-custom") {
        option_val = $really_custom_input.val();
        if (`${option_val}`.match(/\dx$/)) {
          // ...you can't actually type an x; oh well...
  ...
  if (!show_thumbnail) {
    ...
    if (!$thumbnail_window) {
      ...
      new ResizeObserver((entries) => {
        ...
        if (width && height) {
          // If it's hidden, and then shown, it gets a width and height of 0 briefly on iOS. (This would give IndexSizeError in drawImage.)
...
function reset_canvas_and_history() {
  ...
  current_history_node.image_data = main_ctx.getImageData(
    0,
    0,
    main_canvas.width,
    main_canvas.height
...
function update_title() {
  document.title = `${file_name} - ${
    is_pride_month ? "June Solidarity " : ""
  }${localize("Paint")}`;

  if (is_pride_month) {
    $("link[rel~='icon']").attr(
      "href",
      "./images/icons/gay-es-paint-16x16-light-outline.png"
    );
  }
  ...
  for (
    ...
    try {
      ...
      const show_progress = ({ loaded, total }) => {
        if (is_download) {
          $status_text.text(
            `Downloading picture... (${Math.round((loaded / total) * 100)}%)`
          );
        }
      };
      ...
      if (is_download) {
        console.log(
          `Try loading image from URI (${index_to_try + 1}/${
            uris_to_try.length
          }): "${uri_to_try}"`
        );
  ...
  read_image_file(file, (as_image_error, image_info) => {
    if (as_image_error) {
      AnyPalette.loadPalette(file, (as_palette_error, new_palette) => {
        ...
        window.console &&
          console.log(
            `Loaded palette: ${palette.map(() => `%c█`).join("")}`,
...
function apply_file_format_and_palette_info(info) {
  ...
  if (info.palette) {
    window.console &&
      console.log(
        `Loaded palette from image file: ${info.palette
          .map(() => `%c█`)
          .join("")}`,
    ...
    selected_colors.background =
      palette.length === 14 * 2 ? palette[14] : palette[1]; // first in second row for default sized palette, else second color (debatable behavior; should it find a dark and a light color?)
    $G.trigger("option-changed");
  ...
  $w.$Button(
    ...
    () => {
      ...
      if (uris.length > 0) {
        ...
        show_error_message(
          "Invalid URL. It must include a protocol (https:// or http://)"
        );
      }
    },
    { type: "submit" }
...
async function confirm_overwrite_capability() {
  ...
  if (result === "overwrite") {
    acknowledged_overwrite_capability = $window.$content
      .find("#dont-ask-me-again-checkbox")
      .prop("checked");
    try {
...
function file_save_as(
  ...
  systemHooks.showSaveFileDialog({
    ...
    defaultPath:
      typeof system_file_handle === "string" ? system_file_handle : null,
...
function show_file_format_errors({ as_image_error, as_palette_error }) {
  ...
  if (as_palette_error) {
    ...
    if (as_palette_error.errors) {
      details = `<ul dir="ltr">${as_palette_error.errors
        .map((error) => {
          ...
          if (format && error.error) {
            return `<li><b>${escape_html(`${format.name}`)}</b>: ${escape_html(
              uppercase_first(error.error.message)
            )}</li>`;
          }
      ...
      // Fallback for unknown errors
      details = `<p>${escape_html(
        as_palette_error.message || as_palette_error
      )}</p>`;
    }
    html += `
			<details>
...
function show_about_paint() {
  ...
  if (is_pride_month) {
    $("#paint-32x32").attr(
      "src",
      "./images/icons/gay-es-paint-32x32-light-outline.png"
    );
  }

  $about_paint_window.$content
    .append($about_paint_content.show())
    .css({ padding: "15px" });
  ...
  fetch(url)
    ...
    .then((text) => {
      ...
      function entries_contains_update($entries, id) {
        return $entries
          .get()
          .some((el_from_this_version) => id === el_from_this_version.id);
      }

      // @TODO: visibly mark entries that overlap
      const entries_newer_than_this_version = $latest_entries
        .get()
        .filter(
          (el_from_latest) =>
...
function paste(img_or_canvas) {
  if (
    img_or_canvas.width > main_canvas.width ||
    img_or_canvas.height > main_canvas.height
  ) {
    showMessageBox({
      message:
        localize("The image in the clipboard is larger than the bitmap.") +
        "\n" +
  ...
  try {
    ...
    gif.on("finished", (blob) => {
      ...
      $win.$Button(localize("Save"), () => {
        $win.close();
        sanity_check_blob(blob, () => {
          const suggested_file_name = `${file_name.replace(
            /\.(bmp|dib|a?png|gif|jpe?g|jpe|jfif|tiff?|webp|raw)$/i,
            ""
          )} history.gif`;
          systemHooks.showSaveFileDialog({
            ...
            defaultPath:
              typeof system_file_handle === "string"
                ? `${system_file_handle.replace(
                    /[/\\][^/\\]*/,
                    ""
                  )}/${suggested_file_name}`
                : null,
            ...
            formats: [
              {
                formatID: "image/gif",
                mimeType: "image/gif",
                name: localize("Animated GIF (*.gif)").replace(
                  /\s+\([^(]+$/,
                  ""
                ),
    ...
    for (const frame_history_node of frame_history_nodes) {
      ...
      if (frame_history_node.selection_image_data) {
        const selection_canvas = make_canvas(
          frame_history_node.selection_image_data
        );
        gif_canvas.ctx.drawImage(
          selection_canvas,
          frame_history_node.selection_x,
          frame_history_node.selection_y
        );
      }
...
function undoable({ name, icon, use_loose_canvas_changes, soft }, callback) {
  ...
  const image_data = main_ctx.getImageData(
    0,
    0,
    main_canvas.width,
    main_canvas.height
  ...
  const new_history_node = make_history_node({
    ...
    selection_image_data:
      selection &&
      selection.canvas.ctx.getImageData(
        0,
        0,
...
function make_or_update_undoable(undoable_meta, undoable_action) {
  if (
    ...
    current_history_node.selection_image_data =
      selection &&
      selection.canvas.ctx.getImageData(
        0,
        0,
...
function show_document_history() {
  ...
  function render_tree_from_node(node) {
    ...
    $entry
      ...
      .text(
        (node.name || "Unknown") +
          (node === root_history_node ? " (Start of History)" : "")
      );
    ...
    if (!navigator.clipboard || !navigator.clipboard.writeText) {
      if (execCommandFallback) {
        ...
        throw new Error(
          ...
          )} ${recommendationForClipboardAccess}`
        );
...
function make_monochrome_pattern(
  lightness,
  rgba1 = [0, 0, 0, 255],
  rgba2 = [255, 255, 255, 255]
) {
  ...
  const pattern_image_data = main_ctx.createImageData(
  ...
  for (let x = 0; x < pattern_canvas.width; x += 1) {
    for (let y = 0; y < pattern_canvas.height; y += 1) {
      ...
      pattern_image_data.data[index + 3] =
        (px_white ? rgba2[3] : rgba1[3]) ?? 255; // handling also 3-length arrays (RGB)
    }
  }
...
function image_attributes() {
  ...
  const $w = (image_attributes.$window = new $DialogWindow(
    localize("Attributes")
  ));
  ...
  const $width_label = $(E("label"))
    .appendTo($main)
    .html(display_hotkey(localize("&Width:")));
  const $height_label = $(E("label"))
    .appendTo($main)
    .html(display_hotkey(localize("&Height:")));
  const $width = $(E("input"))
    .attr({ type: "number", min: 1, "aria-keyshortcuts": "Alt+W W W" })
    .addClass("no-spinner inset-deep")
    .appendTo($width_label);
  const $height = $(E("input"))
    .attr({ type: "number", min: 1, "aria-keyshortcuts": "Alt+H H H" })
    .addClass("no-spinner inset-deep")
  ...
  $main
    ...
    .css({ width: "40px" })
    .on(
      "change keyup keydown keypress pointerdown pointermove paste drop",
      () => {
  ...
  const $units = $(E("fieldset")).appendTo($main).append(`
		...
		<div class="fieldset-body">
			<input type="radio" name="units" id="unit-in" value="in" aria-keyshortcuts="Alt+I I"><label for="unit-in">${display_hotkey(
        localize("&Inches")
      )}</label>
			<input type="radio" name="units" id="unit-cm" value="cm" aria-keyshortcuts="Alt+M M"><label for="unit-cm">${display_hotkey(
        localize("C&m")
      )}</label>
			<input type="radio" name="units" id="unit-px" value="px" aria-keyshortcuts="Alt+P P"><label for="unit-px">${display_hotkey(
        localize("&Pixels")
      )}</label>
		</div>
	`);
  ...
  const $colors = $(E("fieldset")).appendTo($main).append(`
		...
		<div class="fieldset-body">
			<input type="radio" name="colors" id="attribute-monochrome" value="monochrome" aria-keyshortcuts="Alt+B B"><label for="attribute-monochrome">${display_hotkey(
        localize("&Black and white")
      )}</label>
			<input type="radio" name="colors" id="attribute-polychrome" value="polychrome" aria-keyshortcuts="Alt+L L"><label for="attribute-polychrome">${display_hotkey(
        localize("Co&lors")
      )}</label>
		</div>
	`);
  $colors
    .find(`[value=${monochrome ? "monochrome" : "polychrome"}]`)
    .attr({ checked: true });

  const $transparency = $(E("fieldset")).appendTo($main).append(`
		<legend>${localize("Transparency")}</legend>
		<div class="fieldset-body">
			<input type="radio" name="transparency" id="attribute-transparent" value="transparent"><label for="attribute-transparent">${localize(
        "Transparent"
      )}</label>
			<input type="radio" name="transparency" id="attribute-opaque" value="opaque"><label for="attribute-opaque">${localize(
        "Opaque"
      )}</label>
		</div>
	`);
  $transparency
    .find(`[value=${transparency ? "transparent" : "opaque"}]`)
    .attr({ checked: true });
  ...
  $w.$Button(
    ...
    () => {
      ...
      if (monochrome != was_monochrome) {
        ...
        if (monochrome) {
          if (
            monochrome_info.isMonochrome &&
            monochrome_info.presentNonTransparentRGBAs.length === 2
          ) {
            palette = make_monochrome_palette(
              ...monochrome_info.presentNonTransparentRGBAs
            );
          } else {
...
function image_flip_and_rotate() {
  ...
  $fieldset.append(`
		...
		<div class="radio-wrapper">
			<input
				...
				checked
			/><label for="flip-horizontal">${display_hotkey(
        localize("&Flip horizontal")
      )}</label>
		</div>
		<div class="radio-wrapper">
			<input
			...
			/><label for="flip-vertical">${display_hotkey(
        localize("Flip &vertical")
      )}</label>
		</div>
		<div class="radio-wrapper">
			<input
			...
			/><label for="rotate-by-angle">${display_hotkey(
        localize("&Rotate by angle")
      )}</label>
		</div>
	`);
  ...
  for (const label_with_hotkey of ["&90°", "&180°", "&270°"]) {
  ...
  $w.$Button(
    localize("OK"),
    () => {
      const action = $fieldset
        .find("input[name='flip-or-rotate']:checked")
        .val();
      switch (action) {
        ...
        case "rotate-by-angle": {
          let angle_val = $fieldset
            .find("input[name='rotate-by-angle']:checked")
            .val();
          if (angle_val === "arbitrary") {
            angle_val = $fieldset
              .find("input[name='rotate-by-arbitrary-angle']")
              .val();
          }
...
function image_stretch_and_skew() {
  ...
  $fieldset_stretch.append(
    `<legend>${localize("Stretch")}</legend><table></table>`
  );
  ...
  const $RowInput = (
    ...
    const $input = $(E("input"))
      .attr({
        ...
        value: default_value,
        id: input_id,
        "aria-keyshortcuts": `Alt+${get_hotkey(
          label_with_hotkey
        ).toUpperCase()}`,
      })
      .css({
    ...
    $(E("td"))
      ...
      .append(
        $(E("label"))
          .html(display_hotkey(label_with_hotkey))
          .attr("for", input_id)
      );
  ...
  const stretch_x = $RowInput(
    $fieldset_stretch.find("table"),
    "stretch-x",
    localize("&Horizontal:"),
    100,
    "%",
  ...
  const skew_x = $RowInput(
    $fieldset_skew.find("table"),
    "skew-x",
    localize("H&orizontal:"),
    0,
    localize("Degrees"),
    -90,
    90
  );
  const skew_y = $RowInput(
    $fieldset_skew.find("table"),
    "skew-y",
    localize("V&ertical:"),
    0,
  ...
  return new Promise((resolve) => {
    ...
    const select_file_type_from_file_name = () => {
      const extension_match = (
        promptForName ? $file_name.val() : defaultFileName
      ).match(/\.([\w\d]+)$/);
      if (extension_match) {
...
function write_image_file(canvas, mime_type, blob_callback) {
  ...
  if (bmp_match) {
    ...
    const image_data = canvas.ctx.getImageData(
      0,
      0,
      canvas.width,
      canvas.height
    ...
    (arrayBuffer) => {
      ...
      if (detected_type_id === "bmp") {
        ...
        if (colorTable.length >= 2) {
          if (colorTable.length === 2) {
            palette = make_monochrome_palette(
              ...colorTable.map((color) => [color.r, color.g, color.b, 255])
            );
            monochrome = true;
          } else {
          ...
          if (tabs.PLTE.length === 3 * 2) {
            ...
            for (let i = 0; i < palette.length; i++) {
              if (tabs.tRNS && tabs.tRNS.length >= i + 1) {
                palette[i] = `rgba(${tabs.PLTE[i * 3 + 0]}, ${
                  tabs.PLTE[i * 3 + 1]
                }, ${tabs.PLTE[i * 3 + 2]}, ${tabs.tRNS[i] / 255})`;
              } else {
                palette[i] = `rgb(${tabs.PLTE[i * 3 + 0]}, ${
                  tabs.PLTE[i * 3 + 1]
                }, ${tabs.PLTE[i * 3 + 2]})`;
              }
            }
...
function update_from_saved_file(blob) {
  read_image_file(blob, (error, info) => {
    ...
    undoable(
      {
        name: `${localize("Save As")} ${
          format ? format.name : info.file_format
        }`,
...
function show_multi_user_setup_dialog(from_current_document) {
  const $w = $DialogWindow()
    ...
    .addClass("horizontal-buttons");
  $w.$main.html(`
		${
      from_current_document
        ? "<p>This will make the current document public.</p>"
        : ""
    }
		<p>
