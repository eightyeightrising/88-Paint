(() => {
  ...
  window.tools = [
          ...
          for (let i = 0, l = test_image_data.data.length; i < l; i += 4) {
            if (
              ...
              Math.abs(test_image_data.data[i + 2] - fg_rgba[2]) <=
                fill_threshold &&
              Math.abs(test_image_data.data[i + 3] - fg_rgba[3]) <=
                fill_threshold
            ) {
    ...
    },
    {
      ...
      paint_mask(ctx, x, y) {
        // scale airbrush size up if canvas magnification < 1
        const scale = magnification < 1 ? 1 / magnification : 1;
        const r = (airbrush_size * scale) / 2;
    ...
    if (tool.selectBox) {
      ...
      tool.paint = () => {
        ...
        rect_height =
          ~~Math.min(
            main_canvas.height,
            Math.max(drag_start_y, pointer.y + 1)
          ) - rect_y;
      };
      tool.pointerup = () => {
        canvas_handles.show();
