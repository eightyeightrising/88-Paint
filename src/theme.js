((exports) => {
  ...
  function wait_for_theme_loaded(theme, callback) {
    clearInterval(iid);
    iid = setInterval(() => {
      const theme_loaded = getComputedStyle(document.documentElement)
  ...
  function make_grinch_button() {
    ...
    function animate() {
      ...
      button.style.backgroundPosition = `${
        -Math.floor(smile * (num_frames - 1)) * frame_width
      }px 0px`;
    }
    function on_zoom_etc() {
      // scale to nearest pixel-perfect size
      button.style.transform = `scale(${
        Math.max(1, Math.floor(devicePixelRatio)) / devicePixelRatio
      })`;
